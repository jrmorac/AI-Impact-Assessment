...
## Flujo preferido: Web UI

Ejecuta la UI web local desde esta carpeta:
...
El proyecto incluye tres casos sintéticos que muestran profundidades variables de 5 Whys:

- `why1`: caso simple que confirma la causa en Why 1
- `why3`: caso de complejidad moderada que confirma la causa en Why 3
- `why5`: caso complejo que confirma la causa en Why 5

Ejecuta un demo desde esta carpeta:

```text
python src/main.py demo-run --case why3
```

El comando carga automáticamente el caso y su plan, crea una sesión nueva con timestamp cuando no se indica `--session`, y genera tres artefactos: reporte RCA, CSV CAPA compatible con ADO y CSV de casos de prueba compatible con ADO. La UI ofrece el mismo flujo mediante `Run Demo Case`.

Usa `New Session` en la UI antes de iniciar otro análisis manual. Esto limpia la sesión activa para que el siguiente inicio cree una sesión nueva; las sesiones anteriores permanecen disponibles para revisión.

## CLI secundaria y automatización

Los comandos de terminal siguientes exponen el mismo flujo para automatización, CI o usuarios avanzados. No son necesarios para el uso normal.
