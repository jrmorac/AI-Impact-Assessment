...
Version `3.0.0`. Operational guidance is in [README_OPS.md](README_OPS.md), the architecture source is [docs/architecture.mmd](docs/architecture.mmd), and release history is in [CHANGELOG.md](CHANGELOG.md).
...
Run the same checks locally from this folder:

```text
python -m pip install -r requirements-lock.txt
python -m unittest discover -s tests -v
python -m unittest tests.test_v3_golden_regression -v
python -m compileall -q src tests
PYTHONPATH=src python -c "import agents, interactive_rca, io_utils, main, orchestrator, web_app, workflow_agents"
```

GitHub Actions runs these checks on every push and pull request for Python 3.10 and 3.11. The lock artifact is [requirements-lock.txt](requirements-lock.txt); its comments document the `pip download` and `pip hash` commands used to audit the PyYAML 6.0.2 artifact.

Versioned semantic golden expectations are in [tests/golden](tests/golden). [tests/test_v3_golden_regression.py](tests/test_v3_golden_regression.py) executes the bundled synthetic demo inputs and quick plans for `why1`, `why3`, and `why5`. Timestamps and absolute paths are treated as runtime metadata and are not stored in golden fixtures.
