...
class WebBackendFixTests(unittest.TestCase):
    ...
    def test_export_defaults_and_choice_validation(self):
        ...
        capa_path = web_app._export_output_path({}, session_path, "capa")
        testcases_path = web_app._export_output_path({}, session_path, "testcases")
        self.assertEqual(capa_path, web_app.PROJECT_ROOT / "evidence" / "capa_exports" / "synthetic-capa.csv")
        self.assertEqual(
            testcases_path,
...
class InteractiveRcaFixTests(unittest.TestCase):
    ...
    def test_revision_replaces_explicit_node_and_preserves_question(self):
        chain = [
            {
                "why_index": 1,
                "question": "Original Why 1?",
                "answer": "Original answer",
                "evidence_refs": ["old.log"],
            },
            {
                "why_index": 2,
                "question": "Original Why 2?",
                "answer": "Needs revision",
                "evidence_refs": ["weak.log"],
            },
        ]
        with tempfile.TemporaryDirectory() as folder:
