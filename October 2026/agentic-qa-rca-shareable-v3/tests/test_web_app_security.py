...
class WebAppSecurityTests(unittest.TestCase):
    ...
    def test_read_and_write_allowlists_reject_traversal(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(web_app, "READ_ALLOWLIST", (read_root,)), patch.object(
                ...
                self.assertEqual(web_app._resolve_path(str(read_root / "file.json")), (read_root / "file.json").resolve())
                self.assertEqual(
    ...
    def test_error_mapping_is_semantic_and_stable(self):
        cases = (
            (web_app.PathPolicyError("blocked"), 400, "PATH_NOT_ALLOWED"),
            (FileNotFoundError("missing"), 404, "RESOURCE_NOT_FOUND"),
            (RuntimeError("unexpected"), 500, "INTERNAL_ERROR"),
        )
        for error, status, error_code in cases:
            with self.subTest(error_code=error_code):
                handler = FakeHandler()
                web_app.RcaWebHandler._handle_exception(handler, error)
                response = json.loads(handler.wfile.getvalue())
    ...
    def test_lifecycle_log_is_ndjson_and_excludes_payload_content(self):
        records = []
        capture = logging.Handler()
        capture.emit = lambda record: records.append(record.getMessage())
        web_app.LOGGER.addHandler(capture)
        try:
            web_app._log_event("request.error", "test-request", error_code="INVALID_REQUEST")
        finally:
            web_app.LOGGER.removeHandler(capture)
