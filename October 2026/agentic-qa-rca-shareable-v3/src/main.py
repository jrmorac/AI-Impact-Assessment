...
def parse_args(argv: List[str] | None = None) -> argparse.Namespace:
    ...
    if argv and argv[0] in {
        ...
        parser = argparse.ArgumentParser(description="Agentic QA RCA Assistant")
        ...
        demo_parser = subparsers.add_parser(
            "demo-run",
            help="Run one bundled synthetic demo case with a fresh RCA session",
        )
        demo_parser.add_argument(
            "--case",
            choices=["why1", "why3", "why5"],
            default="why3",
            help="Demo depth to run. Default is why3.",
        )
        demo_parser.add_argument(
            "--context",
            default="project-context/baseline-project.yaml",
            help="Path to project context YAML",
        )
        demo_parser.add_argument(
            "--input",
            default="data/input/demo_cases.json",
            help="Path to demo defect input JSON",
        )
        demo_parser.add_argument("--session", help="Optional session path; omitted creates a new session")
        demo_parser.add_argument("--output-report", help="Optional report path")
        demo_parser.add_argument(
            "--role",
            choices=["dev", "qa", "sre", "release-manager"],
            default="qa",
            help="Role-based answer guidance template",
        )
...
def run_demo(
    *,
    ...
    demo_defect_ids = {"why1": "DEMO-WHY1", "why3": "DEMO-WHY3", "why5": "DEMO-WHY5"}
    ...
    result = run_guided_rca(
        ...
        input_path=input_path,
        defect_id=demo_defect_ids[case_name],
        session_path=selected_session,
        role=role,
        output_report_path=selected_report,
